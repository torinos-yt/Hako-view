...
{
	int offset <String uiname = "InstanceID_offset";>;
...
{
    psInputTextured output =  (psInputTextured)0;
	float4x4 w = sbWorld[input.ii + offset];
    //inverse light direction in view space
    float3 LightDirV = normalize(-mul(float4(lDir,0.0f), tV).xyz);

    //normal in view space
     float3 NormV = normalize(mul(mul(mul(input.normalObject.xyz, (float3x3)tWIT),(float3x3)tV), (float3x3)w)).xyz;
	
    //view direction = inverse vertexposition in viewspace
	
    float4 PosV = mul(input.posObject, mul(w, tWV));
